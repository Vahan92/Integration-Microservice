...
describe('GET /keys/:platform', () => {
    ...
    it('it should return an object with data', (done) => {
        chai.request('http://localhost:4500/api/v2')
            ...
            chai_1.expect(res.body).to.include({ "key": "134", "private_key": "135", "client_id": "133" });
            done();
        });
    });
});
...
describe('Updating...', () => {
    ...
    it('it should return privateId and clientKey', (done) => {
        chai.request('http://localhost:4500/api/v2')
            ...
            res.body.should.be.a('object');
            chai_1.expect(res.body).to.have.keys(['status', 'platform']);
            chai_1.expect(res.body).to.include({ "status": "False", "platform": "magento" });
            done();
        });
    });
});
//# sourceMappingURL=/home/armine_ha/Desktop/integration-microservice/dist/server/tests/integrationRouter.test.js.map